...
const TimelineTableRow = ({ log, index, isExpanded, onClick }: TimelineTableRowProps) => {
  ...
  return (
    <TableRow 
      ...
      onClick={onClick}
    >
      <TableCell className="w-8 px-4 py-3 text-center">
        {isExpanded ? (
      ...
      <TableCell className="whitespace-nowrap px-4 py-3 text-slate-300 text-sm font-medium">
      ...
      <TableCell className="whitespace-nowrap px-4 py-3 text-slate-300">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">{log.user_id || 'N/A'}</span>
        </div>
      </TableCell>
      <TableCell className="whitespace-nowrap px-4 py-3 text-slate-300">
        <div className="flex items-center gap-2">
          <User className="h-4 w-4 text-blue-400/80" />
          <span className="text-sm font-medium">{log.target_user_name || 'N/A'}</span>
        </div>
      </TableCell>
      <TableCell className="px-4 py-3 text-slate-300">
        <div className="flex items-center gap-2">
          <FileText className="h-4 w-4 text-blue-400/80 flex-shrink-0" />
          <span className="text-sm font-medium truncate max-w-[300px]">{log.title || ''}</span>
        </div>
      </TableCell>
      <TableCell className="whitespace-nowrap px-4 py-3 text-slate-300">
        <div className="flex items-center gap-2">
          <Monitor className="h-4 w-4 text-blue-400/80" />
          <span className="text-sm font-medium">{log.computer_name || 'N/A'}</span>
        </div>
      </TableCell>
      <TableCell className="px-4 py-3 text-slate-300">
        <span className="text-sm line-clamp-1 max-w-[400px] font-medium">
          {log.description || 'N/A'}
        </span>
      </TableCell>
      <TableCell className="whitespace-nowrap px-4 py-3 text-slate-300">
        <div className="flex items-center gap-2">
          ...
          <code className="text-xs bg-slate-800/50 px-2 py-0.5 rounded font-medium">
            {log.ip_address || 'none'}
          </code>
        </div>
      </TableCell>
