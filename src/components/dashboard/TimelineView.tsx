...
const TimelineView = ({ entityType, entityId, onClose, inSidebar = false }: TimelineViewProps) => {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      ...
      queryParams.append(queryParam.split('=')[0], queryParam.split('=')[1]);
      ...
      return {
        ...
        user_origin_timeline: data.user_origin_timeline || [], // Add this line to handle user_origin_timeline
        pagination: {
  ...
  const allEvents = data?.pages.flatMap((page) => {
    switch (entityType) {
      case "origin":
        return page.user_origin_timeline;
      case "user":
        return page.user_impacted_timeline;
      case "computer":
        return page.computer_impacted_timeline;
      default:
        return [];
    }
  }) || [];
