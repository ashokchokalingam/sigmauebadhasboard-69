import React from "react";
import { AlertTriangle, Computer, User, UserCog } from "lucide-react";

interface EntityHeaderProps {
  totalEntities: number;
  isLoading: boolean;
  type: "users-origin" | "users-impacted" | "computers";
}

const EntityHeader = ({ totalEntities, isLoading, type }: EntityHeaderProps) => {
  const getHeaderContent = () => {
    switch (type) {
      case "users-origin":
        return {
          icon: <User className="h-6 w-6 text-blue-400/80" />,
          title: "Active Users Origin"
        };
      case "users-impacted":
        return {
          icon: <UserCog className="h-6 w-6 text-blue-400/80" />,
          title: "Active Users Impacted"
        };
      case "computers":
        return {
          icon: <Computer className="h-6 w-6 text-blue-400/80" />,
          title: "Active Computers"
        };
      default:
        return {
          icon: <AlertTriangle className="h-6 w-6 text-blue-400/80" />,
          title: "Active Entities"
        };
    }
  };

  const { icon, title } = getHeaderContent();

  return (
    <div className="flex items-center justify-between bg-[#1e2c3d]/60 p-6 rounded-lg backdrop-blur-sm">
      <div className="flex items-center gap-3">
        {icon}
        <span className="text-2xl font-semibold text-blue-100/90">
          {title}
        </span>
      </div>
      <span className="text-lg font-medium text-blue-400/80">
        {isLoading ? "Loading..." : `${totalEntities.toLocaleString()} active`}
      </span>
    </div>
  );
};

export default EntityHeader;