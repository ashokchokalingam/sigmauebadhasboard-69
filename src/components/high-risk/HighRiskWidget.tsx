...
const HighRiskWidget = ({ title, entityType, endpoint, dataKey }: HighRiskWidgetProps) => {
  ...
  if (isLoading) {
    return (
      <div className="bg-[#0A0B0F] border border-indigo-500/10 rounded-xl overflow-hidden h-[500px]">
        <div className="p-4">
  ...
  return (
    <div className="bg-[#0A0B0F] border border-indigo-500/10 rounded-xl overflow-hidden h-[500px] flex flex-col">
      <div className="p-4 bg-gradient-to-r from-indigo-500/5 to-purple-500/5 border-b border-indigo-500/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-red-500/10 border border-red-500/20">
              <AlertTriangle className="h-4 w-4 text-red-400" />
            </div>
            <span className="text-base font-semibold bg-gradient-to-r from-red-400 to-purple-400 bg-clip-text text-transparent">
              {title}
            </span>
          </div>
          <span className="px-2 py-0.5 rounded-full bg-red-500/10 border border-red-500/20 text-xs font-medium text-red-400">
      ...
      <div className="p-4">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-indigo-400/50" />
          <input
            type="text"
            value={searchQuery}
            ...
            className="w-full pl-9 pr-3 py-2 bg-[#0D0E12] rounded-lg
              ...
              text-xs text-indigo-100/90 placeholder:text-indigo-400/30
              transition-all duration-200 focus:outline-none focus:ring-1 focus:ring-indigo-500/30"
          />
        </div>
      </div>

      <div className="px-4 pb-4 space-y-2 overflow-y-auto flex-1 scrollbar-thin scrollbar-thumb-indigo-500/20 scrollbar-track-transparent">
        ...
        {(!filteredEntities || filteredEntities.length === 0) && (
          <div className="text-center py-4">
            <span className="text-indigo-400/60 text-sm">No critical {entityType === 'asset' ? 'assets' : 'users'} found</span>
          </div>
        )}
      </div>
    </div>
