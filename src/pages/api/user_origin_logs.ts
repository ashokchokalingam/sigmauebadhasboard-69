...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { user_origin, title, page = '1', per_page = '500' } = req.query;

  if (!user_origin) {
    return res.status(400).json({ error: 'user_origin is required' });
  }
